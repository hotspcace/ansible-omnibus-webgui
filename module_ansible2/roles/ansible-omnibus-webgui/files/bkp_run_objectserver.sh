...
#		echo "The table_store db does not exist so checking to see what is in place"
#        if [[ -e "$OMNIDB_DIR" ]]
#        then
#                echo "Omnibus DB Directory $OMNIHOME/db/$OBJSRV exists"
#		echo "Deleting /opt/viasat/omnibus/objectserver/db/.initialized in case it exists"
#		rm -f /opt/viasat/omnibus/objectserver/db/.initialized
#        else
...
#
#                echo "Now doing a dbinit to create the objectserver db"
#
#        fi
#		
#		if [[ -L "$OMNIDB_DIR" ]]
...
#
        ...
        #if [[ "$TEST_HOST_ENTRY" == "$container_name" ]]
        #then
        #        echo "Entry for container name $container_name exists"
        #else
        #        echo "Entry for container name $container_name does not exists so this script will exit"
        #        exit 1
        #fi
